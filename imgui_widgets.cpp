...
bool ImGui::Checkbox(ImStrv label, bool* v)
{
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label.Begin, window->DC.LastItemStatusFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));
    return pressed;
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, ImStrv label)
{
    ...
    if (!item_add)
    {
        ...
        IMGUI_TEST_ENGINE_ITEM_INFO(window->DC.LastItemId, label.Begin, window->DC.LastItemStatusFlags | (is_leaf ? 0 : ImGuiItemStatusFlags_Openable) | (is_open ? ImGuiItemStatusFlags_Opened : 0));
        return is_open;
    }
    ...
    if (is_open && !(flags & ImGuiTreeNodeFlags_NoTreePushOnOpen))
        TreePushOverrideID(id);
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label.Begin, window->DC.LastItemStatusFlags | (is_leaf ? 0 : ImGuiItemStatusFlags_Openable) | (is_open ? ImGuiItemStatusFlags_Opened : 0));
    return is_open;
}
...
bool ImGui::BeginMenu(ImStrv label, bool enabled)
{
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label.Begin, window->DC.LastItemStatusFlags | ImGuiItemStatusFlags_Openable | (menu_is_open ? ImGuiItemStatusFlags_Opened : 0));
...
bool ImGui::MenuItem(ImStrv label, ImStrv shortcut, bool selected, bool enabled)
{
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(window->DC.LastItemId, label.Begin, window->DC.LastItemStatusFlags | ImGuiItemStatusFlags_Checkable | (selected ? ImGuiItemStatusFlags_Checked : 0));
    return pressed;
}
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, ImStrv label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    {
        text_pixel_clip_bb.Max.x -= CalcTextSize(ImStrv(TAB_UNSAVED_MARKER), false).x;
